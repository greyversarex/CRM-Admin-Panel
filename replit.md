...
-   **UI/UX**: Dark navy/slate background with electric indigo accent, professional admin cockpit aesthetic. Components built with shadcn/ui. Frontend dynamically adjusts UI elements based on user roles. Internationalization (i18n) primarily in Russian. Header has no global search bar. Artist/Label roles see a "Create Release" button in the header. CSV export/upload buttons are hidden for artist/label roles.
-   **Transfer Track Module**: Localized module for importing existing music catalogs from Spotify, handling UPC conflicts. Available to all roles (admin, manager, label, artist).
-   **User Blocking with Reason**: Admin/manager can block (suspend) users with a mandatory reason. The reason is stored in `users.block_reason` and an email notification is sent to the blocked user automatically. The block dialog requires entering a reason before confirming.
...
-   **ACRCloud Duplicate-Detection Drop (S3)**: A separate path from DSP delivery for checking a release against ACRCloud's fingerprint database. The moderation dialog's ACR card has a "Send to ACRCloud" button that builds a full DDEX ERN-4.3 package (XML + every track's audio + cover — all tracks must have audio) and uploads it to ACRCloud's S3 bucket via the existing `acrcloud_ddex` connector. Records an `acr_check` (engine `acrcloud_ddex`, mode `ddex_drop`) kept `pending` until a moderator records the verdict manually (Unique → clean / Duplicate → matched / Processing → pending), which then refreshes the release risk score. The S3 bucket/region/prefix/party-ids are code defaults (`PARTNER_DELIVERY_DEFAULTS`); the operator only enters the two S3 access keys in Settings → Integrations.
