...
const DEMO: TakedownRequest[] = [
  { id: 1, release: "Осмон",     artist: "Sitora",    upc: "860000123456", reason: "Смена дистрибьютора", dsps: ["Spotify","Apple Music","Deezer"], status: "completed",  submittedAt: "2025-03-15", completedAt: "2025-03-22" },
  { id: 2, release: "Старый EP", artist: "DJ Farrux", upc: "860000654321", reason: "Устаревший контент",   dsps: ["Spotify","YouTube Music"],        status: "processing", submittedAt: "2025-04-20" },
];
...
export default function TakedownRequests() {
  ...
  const [requests, setRequests] = useState<TakedownRequest[]>(DEMO);
  const [open, setOpen]   = useState(false);
  const [confirm, setConfirm] = useState(false);
  const [form, setForm] = useState({
    release: "", artist: "", upc: "",
    reason: "", note: "", dsps: [] as string[],
  });
  ...
  const handleSubmit = () => {
    if (!form.release || !form.artist || form.dsps.length === 0 || !form.reason) {
  ...
  const handleConfirm = () => {
    ...
    setRequests(p => [req, ...p]);
    setForm({ release: "", artist: "", upc: "", reason: "", note: "", dsps: [] });
    setConfirm(false);
                ...
                <CardHeader className="pb-2">
                  <div className="flex items-start justify-between gap-2">
                    <div>
                      ...
                      <CardDescription>{r.artist} · UPC: {r.upc}</CardDescription>
                    </div>
        ...
        <DialogContent className="sm:max-w-lg">
          ...
          <div className="flex flex-col gap-4 py-2">
            <div className="grid grid-cols-2 gap-3">
              ...
              <div className="grid gap-1.5">
                <Label>Исполнитель <span className="text-red-400">*</span></Label>
      ...
      <Dialog open={confirm} onOpenChange={setConfirm}>
        <DialogContent className="sm:max-w-md">
          ...
          <DialogFooter>
            ...
            <Button variant="destructive" onClick={handleConfirm}>Подтвердить снятие</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
    </Layout>
