...
import { resolveLabelTreeIds } from "./label-scope";

type Scope = ReturnType<typeof getDataScope>;

/**
 * Единая проверка видимости релиза для текущего пользователя (fail-closed).
 * Для лейбла релиз «свой», если он привязан к корневому лейблу/его подлейблу
 * напрямую ИЛИ принадлежит артисту внутри того же дерева каталога.
 */
export async function releaseInScope(
  ...
  if (scope.role === "label") {
    ...
    if (r.labelId != null && labelIds.includes(r.labelId)) return true;
    const [a] = await db.select({ labelId: artistsTable.labelId }).from(artistsTable).where(eq(artistsTable.id, r.artistId));
    return a?.labelId != null && labelIds.includes(a.labelId);
  }
  return false;
}
...
export async function labelArtistIds(labelId: number): Promise<number[]> {
  const labelIds = await resolveLabelTreeIds(labelId);
  const rows = await db.select({ id: artistsTable.id }).from(artistsTable).where(inArray(artistsTable.labelId, labelIds));
...
export async function labelReleaseScopeCondition(labelId: number) {
  ...
  const parts: any[] = [inArray(releasesTable.labelId, labelIds)];
