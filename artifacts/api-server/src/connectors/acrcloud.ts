...
export const acrcloudConnector: IConnector = {
  ...
  async testConnection(ctx: ConnectorContext): Promise<ConnectorResult> {
    ...
    try {
      ...
      if (code === 3003) {
        // Ключи валидны, но квота кончилась — реальные сканы упадут.
        // Возвращаем ok=true + unverified=true, чтобы UI показал жёлтый бейдж
        // «Не подтверждено», а не зелёный «Подключено». Так лейбл сразу увидит,
        // что подключение неполное, и пополнит квоту до загрузки релизов.
        return {
          ok: true,
          unverified: true,
          message: `Ключи валидны, но КВОТА ACRCloud ИСЧЕРПАНА (${msg}). Реальные проверки треков падают с ошибкой. Пополните пакет в личном кабинете ACRCloud перед загрузкой релизов.`,
