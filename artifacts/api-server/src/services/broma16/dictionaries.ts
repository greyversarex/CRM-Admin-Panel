...
/**
 * Подстроки названий витрин Broma16 для сопоставления наших id артиста.
 * Словарь outlets Broma16 отдаёт человекочитаемые названия (code пустой),
 * поэтому матчим по названию: Spotify → «Spotify», Apple → «Apple Music, iTunes».
 */
const ARTIST_OUTLET_NAME_HINTS: Record<string, string[]> = {
  spotify: ["spotify"],
  apple: ["apple music", "itunes", "apple"],
};

export type ArtistOutlet = { outlet: string; outlet_id: number; id_outlet_user: string };

/**
 * Собирает массив outlets артиста для Broma16 из Spotify/Apple Artist ID.
 * Каждый элемент: { outlet (название), outlet_id (id витрины из словаря),
 * id_outlet_user (id артиста на витрине) } — см. POST /account/{id}/artist/.
 * Витрины, которых нет в словаре Broma16, пропускаем (не угадываем id).
 */
export async function resolveArtistOutlets(ids: {
  spotifyId?: string | null;
  appleId?: string | null;
}): Promise<ArtistOutlet[]> {
  const wanted: Array<{ key: string; value: string }> = [];
  if (ids.spotifyId && ids.spotifyId.trim()) wanted.push({ key: "spotify", value: ids.spotifyId.trim() });
  if (ids.appleId && ids.appleId.trim()) wanted.push({ key: "apple", value: ids.appleId.trim() });
  if (wanted.length === 0) return [];

  const dict = await getDictionary("outlet");
  if (dict.length === 0) return [];

  const out: ArtistOutlet[] = [];
  for (const w of wanted) {
    const hints = ARTIST_OUTLET_NAME_HINTS[w.key] ?? [w.key];
    const match = dict.find((d) => hints.some((h) => norm(d.name).includes(norm(h))));
    if (!match) continue;
    const outletId = Number(match.externalId);
    if (!Number.isFinite(outletId)) continue;
    out.push({ outlet: match.name, outlet_id: outletId, id_outlet_user: w.value });
  }
  return out;
}
