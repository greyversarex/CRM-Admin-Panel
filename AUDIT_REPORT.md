...
#### 🔴 КРИТИЧНО — ACRCloud: работает только при наличии ключей, без них молча не сканирует
**Файл:** `routes/distribution-extras.ts` стр. 88–91  
Без заполненных credentials в Settings → Интеграции → ACRCloud все запросы на сканирование возвращают `HTTP 503` с сообщением «ACRCloud credentials not configured». В UI это отображается как "не настроен" без явной инструкции по устранению. Ключей нет ни в `.env`, ни в seed-данных.
...
#### ⚠️ СРЕДНЕ — PRO Registration: работает только при наличии credentials, нет реального PRO API
**Файл:** `routes/publishing-extras.ts` стр. 111–187  
Endpoint формирует JSON-payload (CWR-like) и отправляет POST на произвольный `endpoint` из настроек. Логика правильная, но:
1. Без `endpoint` + `apiKey` в Settings → Publishing → PRO — возвращает `HTTP 503`.
2. Реального договора ни с ASCAP, ни с BMI, ни с Songtrust/MLC нет — endpoint должен быть предоставлен этими организациями и является их собственным API. Функция работает технически, но **не может быть проверена без реального аккаунта в PRO-организации**.
3. В seed-данных и defaults нет ни одного PRO-endpoint — кнопка «Зарегистрировать» всегда вернёт 503 в текущей установке.
...
| ⚠️ СРЕДНЕ | ACRCloud — работает только при наличии credentials, иначе 503 | `routes/distribution-extras.ts:88-91` |
...
| ⚠️ СРЕДНЕ | PRO Registration (ASCAP/BMI/MLC): без реального endpoint → 503 | `routes/publishing-extras.ts:111-119` |
