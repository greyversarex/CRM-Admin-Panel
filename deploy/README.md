...
### 7. Локальное хранилище файлов (для уже работающего сервера)

Если сервер уже развёрнут по старой схеме и теперь нужно включить загрузку
обложек/треков/KYC через локальную ФС — запустите один раз:

```bash
sudo bash /var/www/tajikmusic/deploy/setup_storage.sh
```

Скрипт сам:
- создаст каталог `/var/lib/tajikmusic/uploads` с правильными правами,
- пропишет `LOCAL_STORAGE_ROOT` в `.env`,
- поднимет `client_max_body_size` в nginx до `250M`,
- проверит конфиг и перезагрузит nginx,
- перезапустит API через pm2.

Все правки делаются с бэкапами (`.bak.<дата>`). Скрипт можно запускать повторно — он ничего не сломает. Для нового деплоя «с нуля» (`2_deploy.sh`) запускать его не нужно — каталог создаётся автоматически.
